...
void Detector::createMaterials()
{
  ...
  // Shared tracking parameters for the passive materials below
  float tmaxfdPas = 0.1;
  float stemaxPas = 1.0;
  float deemaxPas = 0.1;
  ...
  if (stopHit) {
    ...
    if (subDetID == 1) {
      if (trkPars.layoutMLOT == o2::trk::eMLOTLayout::kSegmented) {
        ...
        // The realistic OT builds stave/half-stave/module as assembly volumes, for
        // which fMC->CurrentVolOffID copy numbers are unreliable (all read 0). Resolve
        // the indices from the TGeo node path at the hit mid-point instead.
